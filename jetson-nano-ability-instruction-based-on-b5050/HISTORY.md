...
A 12 MP phone photo can still trip the GPU watchdog because Qwen-VL-style **dynamic-resolution** vision encoders emit a patch count proportional to input area (14×14 patches over the whole image, not a fixed 224×224 thumbnail), and tens of thousands of patches make a single vision-encoder kernel run past the Jetson's ~2 s deadline. The principled cap is to pass `--image-max-tokens 256` (≈ 1024 patches ≈ 448×448 input — the flag counts post-spatial-merge LLM tokens, four patches per token), which sits comfortably below our confirmed-working 512×512 baseline; manually downsampling the image to ~512 px is an equivalent fallback. See `INSTALL.md` § "Vision: cap the patch count for big photos" for the full table of patches vs LLM tokens vs feasibility.
